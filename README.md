...
* In flight display mode, if you selected IBG or GPS for the data logging option, btnM toggles logging on and off. If you selected high-speed IBG logging the display will show 'I' if logging, 'i' if not logging. If you selected GPS track logging, the display will show 'G' if logging, 'g' if not logging. If you selected NONE for the logging option, btnM has no effect. Bear in mind that you selected high-speed IBG data logging and toggle it 'on', the 128Mb flash will be filled up in approximately 13minutes ! If the flash is full, logging stops.
* In flight display mode, btnR toggles the audio on and off. You will see the status reflected on the display with the speaker icon. 
...
* Glide ratio = horizontal velocity from gps / vertical velocity from barometric altitude change. If climbing, the glide ratio field displays '+'. There is a damping filter to reduce jitter in the
