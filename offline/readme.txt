...
# "ESP32GpsVario", and download the binary datalog file using the url  http://192.168.4.1/datalog
...
# the ibg datalogs also contain gps fix data, so this code can be used to convert the gps fix data into
# gpx files that you can open in Google Earth or other 3D track visualization software. Remember to
...
# than the specified value in milliseconds (e.g. 300). Also splits wherever there is a change from 
# ibg log to gps track log or vice versa.
gcc -o logsplit logsplit.c
./logsplit ~/Downloads/datalog 300     

# testmar2 is an example of offline analysis of a high-speed IBG data log. 
